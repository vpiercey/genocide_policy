...
#Read in EWP
sra_2022 <- read_csv("Research/Genocide Resilience/FSI_EWP/data/sra_2022.csv")
View(sra_2022)

#Get rid of everything except COW and indicators
fsi_2022 <- subset(fsi_2022, select = -c(3,4,5))
sra_2022 <- subset(sra_2022, select = -c(2,4,5))

#Check to see if variance of any column for EWP is 0 (not a concern for FSI)
for (i in 3:33) {
  if (var(sra_2022[,i])==0){print(i)}
  if (var(sra_2022[,i]) > 0) {print("No Problem")}
}
...
sra_2022 <- subset(sra_2022, select = -c(15))

#We should be good to join the two dfs now! 
fsi_ewp_2022 <- merge(fsi_2022, sra_2022, by = "COW", all = TRUE) #the last gives us all rows even if not in both, so we can see if there is anything missing.

#Now we'll check for problems in the list of countries.
v <- c(1,2,15) #these are column numbers with countries and COW
countries <- fsi_ewp_2022[,v]

#We can see that every EWP country is in FSI (even if different names), but the converse is not true.
...
#Remove the NAs
fsi_ewp_2022 <- na.omit(fsi_ewp_2022)
nrow(fsi_ewp_2022) #check that the number is correct. Should be 178 original rows minus 16 NAs.
...
fsi_ewp_2022_final <- subset(fsi_ewp_2022, select = -c(1, 15))
rownames(fsi_ewp_2022_final) <- fsi_ewp_2022_final$Country
fsi_ewp_2022_final <- subset(fsi_ewp_2022_final, select = -c(1))

#now we make the heatmap of correlations. Columns 13 through 42 are EWP, columns 1 through 12 are FSI.
x <- fsi_ewp_2022_final[,1:12]
y <- fsi_ewp_2022_final[,13:42]
corr_2022 <- cor(x,y)
heatmap(corr_2022)
...
fsi_ewp_2022_final_std <- scale(fsi_ewp_2022_final) #have to standardize in order to do clustering - otherwise different scales will distort distances
fsi_ewp_2022_final_std <- t(fsi_ewp_2022_final_std) #Hclust clusters based on rows. Since we want to cluster the indicators, we need to make the indicators the rows using the transpose "t()" function.
#Note: the scale() function standardizes columns, and we need standardized rows for the clustring. So we had to use scale() first, then transpose.

clusters_2022 <- hclust(dist(fsi_ewp_2022_final_std))
plot(clusters_2022)